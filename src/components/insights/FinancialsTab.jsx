      ...
      {activeSection === 'bank' && (bank || ledger) && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {ledger?.transactions && ledger.transactions.length > 0 && (() => {
            const DEPOSIT_CATEGORIES = ['Etsy Payout'];
            const txns = ledger.transactions;
            const currentBalance = ledger.current_balance || bank?.balance || 0;

            // Compute starting balance by reversing all transactions
            const netChange = txns.reduce((sum, t) => {
              const isDeposit = DEPOSIT_CATEGORIES.includes(t.category);
              return sum + (isDeposit ? (t.amount || 0) : -(t.amount || 0));
            }, 0);
            let runningBal = currentBalance - netChange;

            return (
            <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontWeight: '600' }}>Recent Transactions ({txns.length})</span>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                  <thead style={{ background: '#f8fafc', position: 'sticky', top: 0 }}>
                    <tr>
                      ...
                      <th style={{ padding: '12px 16px', textAlign: 'right', fontWeight: '500', color: '#64748b' }}>Money In</th>
                      <th style={{ padding: '12px 16px', textAlign: 'right', fontWeight: '500', color: '#64748b' }}>Money Out</th>
                      <th style={{ padding: '12px 16px', textAlign: 'right', fontWeight: '500', color: '#64748b' }}>Balance</th>
                    </tr>
                  </thead>
                  <tbody>
                    {txns.map((txn, idx) => {
                      const isDeposit = DEPOSIT_CATEGORIES.includes(txn.category);
                      runningBal += isDeposit ? (txn.amount || 0) : -(txn.amount || 0);
                      return (
                      <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9', background: isDeposit ? 'rgba(16, 185, 129, 0.03)' : 'transparent' }}>
                        ...
                        <td style={{ padding: '12px 16px', textAlign: 'right', color: '#10b981', fontWeight: '500' }}>
                          {isDeposit ? `+$${txn.amount?.toLocaleString('en-US', { minimumFractionDigits: 2 })}` : ''}
                        </td>
                        <td style={{ padding: '12px 16px', textAlign: 'right', color: '#ef4444', fontWeight: '500' }}>
                          {!isDeposit ? `-$${txn.amount?.toLocaleString('en-US', { minimumFractionDigits: 2 })}` : ''}
                        </td>
                        <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: '600', color: runningBal >= 0 ? '#1a1a2e' : '#ef4444' }}>
